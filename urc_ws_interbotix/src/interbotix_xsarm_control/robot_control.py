...
class ArmController(Node):
    ...
    def __init__(self):
        ...
        ### PARAMS ###
        self.alpha = 0.0005#0.055                    # scaling factor for controlling the arm
   ...
   # def on_key_press(self, key):
    ...
    def listener_callback(self, msg : Imu):
        ...
        if self.enable_imu:
            threshold = 0.1
            self.get_logger().info(f"Moving arm: {self.target_positions}")
            if abs(msg.angular_velocity.z) > threshold:
                self.target_positions[0] += self.alpha * msg.angular_velocity.z
            if abs(msg.angular_velocity.y) > threshold:
